...
type Props = { settings: Settings; courses: string[][]; lessons: string[][]; articles: string[][]; books: string[][]; search: string; setSearch: (value: string) => void; results: Result[]; ownerSession: boolean; onOpenAdmin: () => void | Promise<void>; onOpenInterest: () => void; onSubscribeNewsletter: (event: FormEvent<HTMLFormElement>) => void | Promise<void>; newsletterSending: boolean };
...
export default function PulseHomepage({ settings, courses, lessons, articles, books, search, setSearch, results, ownerSession, onOpenAdmin, onOpenInterest, onSubscribeNewsletter, newsletterSending }: Props) {
  ...
  useEffect(() => { const onScroll = () => setShowTop(window.scrollY > 680); onScroll(); window.addEventListener("scroll", onScroll, { passive: true }); return () => window.removeEventListener("scroll", onScroll); }, []);
  ...
  const learningItems = useMemo(() => [...courses.map(([title]) => ({ id: `course:${slugify(title)}`, title, href: `/courses/${encodeURIComponent(slugify(title))}`, kind: "دورة" })), ...lessons.map(([title]) => ({ id: `lesson:${slugify(title)}`, title, href: `/lessons/${encodeURIComponent(slugify(title))}`, kind: "درس" }))], [courses, lessons]);
