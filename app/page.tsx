...
export default function Home() {
  ...
  return <div className="redesign-site">
    ...
    <main id="top">
      {settings.showHero && <section className="hero"><div className="hero-copy"><div className="hero-topline"><span className="hero-index">01</span><i /><small>{settings.homeLeadKicker}</small><span className="hero-live"><b /> متاح الآن</span></div><p className="kicker">{settings.homeLeadKicker}</p><h1><span>{settings.homeLeadTitle}</span></h1><div className="hero-story"><p>{settings.homeLeadText}</p><div className="hero-actions"><MemberAccount initialMode="signup" label={settings.homeLeadPrimaryCta || "سجل الآن"} hideWhenAuthenticated className="primary hero-account-cta" /></div><div className="hero-trust-line"><span>دروس مركزة</span><i /> <span>مجتمع عالمي</span><i /> <span>تعلم مستمر</span></div></div></div><div className="hero-visual-column">{settings.showWorldGlobe && <WorldGlobe compact visualOnly showCountryLabels />}<div className="hero-stat-rail"><div><span>مسارات</span><b>{courses.length}</b><small>تعلم مرتب</small></div><div><span>مواد</span><b>{lessons.length + articles.length + books.length}</b><small>قراءة واستماع</small></div><div><span>العالم</span><b>∞</b><small>أثر يتصل</small></div></div></div></section>}
      ...
      <HomeSpotlight course={courses.find((row) => isFeatured(row, 5))} lesson={lessons.find((row) => isFeatured(row, 4))} article={articles.find((row) => isFeatured(row, 5))} />
